...
const schoolLocation: { lat: number; lng: number } = {
  lat: 16.1689307,
  lng: 80.8048806,
};

const waterWells: { name: string; location: [number, number]; year: number }[] =
  [
    {
      name: 'Pallapatla',
...
const Map = () => {
  ...
  const renderWaterWells = () => {
    return waterWells.map((w, index) => (
      <Marker key={w.name + index} position={w.location}>
        <Popup>
          <div>
            {w.name} ({w.year})
          </div>
  ...
  return (
    <div>
      ...
      <MapContainer center={schoolLocation} zoom={12} scrollWheelZoom={false}>
        <TileLayer
